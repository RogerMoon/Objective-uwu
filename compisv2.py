...
# 			| PR_true
# 			| PR_false
# 			| firstIndex moreDimIndex TO_CORCIERRA'''
...
def p_var1(p):
 	...
 	if not p[3] in dir_func[actual_scope]['scope']:
 		...
 		print(actual_scope+ " "+ p[3])
 		print(dir_func[actual_scope]['scope'][p[3]])
 		p[0] = p[3]
 		global toDim
 		toDim = p[3]
...
# def p_arrayIndex(p):
# 	'''arrayIndex : firstIndex moreDimIndex TO_CORCIERRA 
# 				  | empty'''
...
# 		index = pop_pilaO()
# 		result_type = semantic_check(pop_pType(), 'NUM', '=')
...
# 				memoria[nextT]= 0
# 				temp2 = '(' + str(nextT) + ')'
# 				chorizo = actual_scope+'/'+p[1]
# 				add_quad('DIRBASE',temp1,chorizo,temp2)
# 				add_pilaO(temp2)
# 				add_pType(dir_func[varscope]['scope'][p[1]].get('type'))
# 			else:
# 				nextT = nextTemp('NUM')
# 				memoria[nextT]= 0
# 				temp1 = '(' + str(nextT) + ')'
...
# def p_moreDimIndexCreate(p):
# 	'''moreDimIndex : TO_COMA exp moreDimIndex
# 					 | empty'''
